...
#     #Transposes the data
#     df = df.T
#     name_space.value = df._get_value("Period 1", disk_number-1)
    temp_out_of_class.append(disk_number)
    current_name = disk_number
...
def check_times():
    ...
    SHEET_ID = '19hGHr8-0T0pFY_MSwgHA91Sr0qFBq-qLHxfBgVM1Vkw'
    #SHEET_NAME is the name of the tab
    SHEET_NAME = 'Names'
    url = f'https://docs.google.com/spreadsheets/d/{SHEET_ID}/gviz/tq?tqx=out:csv&sheet={SHEET_NAME}'
    df = pd.read_csv(url)
    ...
    student_names = []
    for i in range(40):
        student_names.append(df._get_value(class_number, i))
    combo.clear()
...
hour_text = Text(app, text = 'Pick Your Name', color = 'white')
...
for i in range(40):
    student_names.append(df._get_value(class_number, i))
print(student_names)
combo=Combo(app, options =student_names, command = specific_names)
combo.text_size = 8
...
reset_class = PushButton(app, text = 'Reset Class', command = check_times)
app.display()
